...
C     max is the maximum number of points in each data file.
      ...
      program fdpfil
      parameter (max=130000, N=2**8)
      integer g, a
      real arr1(max),arr2(max),arr3(max),SS(6,6), P(N)
      real sac1(max),sac2(max),sac3(max), d(6), v(6,6), Pg(max)
...
C     file names to be used.
      ...
      character*11 knimeo
      character*9 pgname
C      write(*,*) 'enter sac filenames'
C      read(*,*) kname, knome, knime
C      write(*,*) 'enter output sac filenames (4)'
C      read(*,*) knameo, knomeo, knimeo, pgname
C      write(*,*)'inputs are ',kname,' ',knome,' ',knime
C      write(*,*)'outputs: ',knameo,' ',knomeo,' ',knimo,' ',pgname
       kname = 'tmpu.sac'
       knome = 'tmpv.sac'
       knime = 'tmpw.sac'
C       open (unit = 5, file = 'pgl')
...
C       write(*,*) 'writing sac arrays'
         sac1(l)=ptemp1(1+(N-1)/2)/N
         sac2(l)=ptemp2(1+(N-1)/2)/N
         sac3(l)=ptemp3(1+(N-1)/2)/N
         Pg(l)=P(3)**g
...
C     sac output files specified at the beginning of the program.
      ...
      write(*,*) 'calling wsac1'
      call wsac1('out.tmpu.sac',sac1,nlen,beg,del,nerr)
      call wsac1('out.tmpv.sac',sac2,nlen,beg,del,nerr)
      call wsac1('out.tmpw.sac',sac3,nlen,beg,del,nerr)
      call wsac1('pgout.sac',Pg,nlen,beg,del,nerr)
C      close(5)
      end
